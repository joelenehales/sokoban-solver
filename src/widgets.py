...
# From: https://github.com/xbandrade/sokoban-solver-generator/blob/main/src/widgets.py
...
def sidebar_widgets(window):
	prev_button = Button(
		...
		borderColor=BORDER_COLOUR, borderThickness=BORDER_THICKNESS,
	)
	...
	bfs_button = Button( # TODO: Change this to DFS
...
class Label:
	def __init__(self, window, text, x, y, font_size, transparency=False, color=BORDER_COLOUR):
		self.x = x
		self.y = y
	...
	def set_text(self, new_text, font_size, color=BORDER_COLOUR):
		self.font = pygame.font.SysFont(FONT, font_size, bold=True)
		self.image = self.font.render(new_text, 1, color)
		self.draw()

	def set_moves(self, new_text, font_size, color=BORDER_COLOUR):
		self.font = pygame.font.SysFont(FONT, font_size, bold=True)
		self.image = self.font.render(new_text, 1, color)
...
class LevelClear(Label):
	def __init__(self, window, text, x=256, y=192, font_size=60, color=BORDER_COLOUR):
